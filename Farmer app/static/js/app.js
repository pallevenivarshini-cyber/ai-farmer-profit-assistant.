...
function renderBestMarketHighlight(data) {
    ...
    container.innerHTML = `
        ...
        <div class="best-market-text">
            <h3>${t('bestMarket')}: ${bm.market}</h3>
            <p>${t('sellYour')} <strong>${cropName}</strong> ${t('atPrice')} <span class="bm-price">₹${bm.price}/kg</span> ${t('inMarket')} ${bm.market} (${transLocation}) ${t('forMaxProfit')} <span class="bm-price" style="color: ${totalProfit > 0 ? 'var(--accent-green)' : 'var(--accent-red)'}">₹${totalProfit}</span></p>
        </div>
    `;
}
...
function renderMarketPrices(data) {
    ...
    if (!data.current_prices || data.current_prices.length === 0) {
        container.innerHTML = '<p style="color: var(--text-muted); padding: 16px;">No market data available</p>';
        return;
    }
...
function renderProfitCard(data) {
    ...
    if (!data.profit) {
        container.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: 16px;">Enter quantity and cost to see profit</p>';
        return;
    }
...
function renderAlerts(data) {
    ...
    if (!data.alerts || data.alerts.length === 0) {
        container.innerHTML = '<div class="alert-item info"><span class="alert-icon">✅</span><span class="alert-text">No alerts at this time</span></div>';
        return;
    }
    ...
    data.storage.forEach((facility, index) => {
        ...
        item.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 16px;">
                <div>
                    <h3 style="margin: 0 0 4px 0; font-size: 1.15rem; color: #f0f4f8;">${facility.name}</h3>
